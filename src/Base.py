    ...
    def __call__(self, batch):
        ...
        for mini_batch in batch:
            for k, v in mini_batch.items():
                batch_in[k].append(v)
        sen1_encoding = self.tokenizer(batch_in["sen1_text"], padding="longest", max_length=self.config["FIX_LENGTH"], truncation=True)
        sen2_encoding = self.tokenizer(batch_in["sen2_text"], padding="longest", max_length=self.config["FIX_LENGTH"], truncation=True)
...
def predict(args):
    ...
    data = DP.prepare_examples(args.test_path, True)
...
def train(args):
    ...
    for step in range(args.fold):
        ...
        if args.train_path.endswith(".csv"):
            train_data = DP.prepare_examples(args.train_path)
        else:
            train_data = DP.prepare_examples(args.train_path + f"_fold_{step}.csv")
